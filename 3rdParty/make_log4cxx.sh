...
build_log4cxx(){
    ...
    cd $LOG4CXX
    ./configure --prefix=$CUR_DIR/package_log4cxx --with-apr=/usr --with-apr-util=/usr
    cp ../inputstreamreader.cpp ./src/main/cpp/
    cp ../socketoutputstream.cpp ./src/main/cpp/
    cp ../console.cpp ./src/examples/cpp/
    make
    make install
	#切换到package_log4cxx同级目录
    cd ../../
    cp -rf package_log4cxx/include log4cxx/
    mkdir -p log4cxx/lib/
    cp -f package_log4cxx/lib/liblog4cxx.so* log4cxx/lib/
}
