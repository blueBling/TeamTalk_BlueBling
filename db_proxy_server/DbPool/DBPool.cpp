...
CResultSet::CResultSet(MYSQL_RES* res)
{
	...
	int num_fields = mysql_num_fields(m_res); //返回结果集中字段的数
	MYSQL_FIELD* fields = mysql_fetch_fields(m_res); //返回包含字段定义信息的对象
	for(int i = 0; i < num_fields; i++)
	{
	   m_key_map.insert(make_pair(fields[i].name, i)); //将各个字段名和编号映射到map
	}
}
...
{
	m_row = mysql_fetch_row(m_res); //检索一个结果集合的下一行
	if (m_row) {
		return true;
	} else {
		return false;
...
bool CPrepareStatement::ExecuteUpdate()
{
	...
	if (mysql_stmt_bind_param(m_stmt, m_param_bind)) { //为待执行的语句绑定参数
		log("mysql_stmt_bind_param failed: %s", mysql_stmt_error(m_stmt));
		return false;
	}

	if (mysql_stmt_execute(m_stmt)) { //执行语句
		log("mysql_stmt_execute failed: %s", mysql_stmt_error(m_stmt));
		return false;
	}

	if (mysql_stmt_affected_rows(m_stmt) == 0) { //获取改变的行数
...
{
	return mysql_stmt_insert_id(m_stmt); //返回执行INSERT或UPDATE自动生成的id(AUTO_INCREMENT)
}
