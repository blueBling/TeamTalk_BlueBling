...
static ConnMap_t g_proxy_conn_map; // 保存每个socket fd的连接
static UserMap_t g_uuid_conn_map;  // 每个连接对应唯一一个uuid
static CHandlerMap* s_handler_map; // 每个不同的commandId对应不同的执行任务

uint32_t CProxyConn::s_uuid_alloctor = 0;
CLock CProxyConn::s_list_lock;
list<ResponsePdu_t*> CProxyConn::s_response_pdu_list; // 主线程待发送回复消息
